...
class Player {
  constructor(x, y, id) {
    ...
    const canv = new drawTool('mycanvas');
  ...
  jump() {
    const bopLeftRight = (this.collisionRIGHT && this.collisionLEFT)
  ...
  update() {
    ...
    const leftRight = (this.collisionRIGHT && this.collisionLEFT)
        || (!this.collisionLEFT && !this.collisionRIGHT);
  ...
  kill() {
    if (localStorage.getItem('matchStarted')) {
      post('/api/match/endMatch', { id: localStorage.getItem('matchId') }, () => {
