class Game {
  ...
  update() {
    ...
    if (this.players[0] && this.players[0].posY >= d.height + 100) {
      this.me.killOther();
    }
  ...
  generatePlatforms() {
    ...
    for (let i = 0; i < 100; ++i) {
      const randomVal = random(i * this.seed + 1);

      const nPlatforms = Math.floor(randomVal * 3);
      ...
      const index = Math.floor(type.length * random(i * 2 * this.seed));
      const level = type[index];

      const decrease = 1 - 0.1 * Math.floor(i / 10);
      const blocks = [];
  ...
  drawPlatforms() {
    for (let i = 0; i < this.platforms.length; ++i) {
      ...
      for (let j = 0; j < level.length; ++j) {
        ...
        if (current.y >= 0 && current.y <= d.height + current.h) { current.show(); }
      }
    }
  }
